...
int sw_recv(struct sw_dev_info *info, u8 data[], int len)
{
...
int sw_init(struct sw_dev_info *dev)
{
	char device[40];
...
void sw_exit(struct sw_dev_info *dev)
{
	if (dev->fd <= 0)
		return;
	usleep(10 * 1000);
...
{
	struct sw_dev_info *info = fd;

	return ioctl(info->fd, DEV_IOC_DLR, req);
}

#ifdef USE_NET_IOCTL_
struct dev_info {
	int sock;
	char name[20];
};
...
static int sw_get_port_cfg(void *fd,
...
static int sw_set_port_cfg(void *fd,
...
int get_port_learn(void *fd,
	int port, int *get)
{
	return sw_get_port_cfg(fd, port, get,
		SP_LEARN);
}
...
int set_port_learn(void *fd,
	int port, int set)
{
	return sw_set_port_cfg(fd, port, set,
		SP_LEARN);
}
