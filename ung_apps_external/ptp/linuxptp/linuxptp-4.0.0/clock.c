			   ...
			   const char *phc_device)
{
	...
	if (c->clkid != CLOCK_INVALID) {
		...
		/* Want to make the initial drift difference to exceed. */
		c->diff_fadj = KSZ_ACCURATE_PATH_DELAY_DRIFT * 2;
		if (fadj < 0)
			c->last_fadj = c->diff_fadj - fadj;
		else
			c->last_fadj = -(c->diff_fadj + fadj);
#endif
		...
		for (i = 0; i < N_POLLFD; i++) {
			if (cur[i].revents & (POLLIN|POLLPRI|POLLERR)) {
				...
				if (cur[i].revents & POLLERR) {
					...
					cur[i].revents &= ~(POLLIN|POLLPRI|POLLERR);
					if (event == EV_POWERUP) {
						break;
					}
				}
...
void clock_path_delay(struct clock *c, tmv_t req, tmv_t rx)
{
	...
	if (c->diff_fadj >= KSZ_ACCURATE_PATH_DELAY_DRIFT) {

		/* Need valid indication in clock_synchronize(). */
		tsproc_set_delay(c->tsproc, tmv_zero());
		return;
	}
#endif
...
#ifdef KSZ_1588_PTP
	/* Can still calculate path delay when previous Sync information is
	 * reset and wait for next Sync.
	 */
	c->no_delay = 1;
#endif
...
enum servo_state clock_synchronize(struct clock *c, tmv_t ingress, tmv_t origin)
{
...
#ifdef KSZ_1588_PTP
	/* Previous Sync information is reset but there is still Delay_Resp
	 * information to calculate path delay.
	 */
	if (c->no_delay) {
		tsproc_update_delay(c->tsproc, &c->path_delay);
	}
#endif
	 ...
	 */
	c->diff_fadj = abs(c->last_fadj - adj);
	if (tmv_is_zero(c->path_delay) &&
	    (SERVO_UNLOCKED == c->servo_state ||
	     SERVO_LOCKING == c->servo_state) && state == SERVO_JUMP) {
